...
Both pages are generated by `build.py` from plain text files, so no HTML editing is needed.

**Shop** — `assets/shop/products.txt`. One line per product: id, group, name, price,
image, blurb, and an optional add-on. Put a real number in the price column, or `POA`
to show "Price on call". Put `soon` in the image column and the card shows a
"photo coming" tile until you send a photo.

**Gallery** — drop photos in `assets/gallery/`, and add an optional caption line to
`assets/gallery/captions.txt`.

After either change, run `python3 build.py` and push.

The shop does not take payment. It builds an order and hands it over as a text
message to 0429 543 435. Taking cards would need a payment provider account
(Stripe, PayPal or a Shopify buy button) and its public key.
